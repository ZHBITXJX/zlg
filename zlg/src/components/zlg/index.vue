<template>
  <div class='store'>
    <div class="header">
      <div class="header-title">重力柜列表</div>
      <div class="header-adddata">
        <div @click='showzlgInput1' style="font-size: 16px">
    ...
    <div class="center">
      <table  class="table">
        ...
        <tr v-for='(item,index) in zlgdata' >
          ...
          <td v-else>未使用</td>
          <td>
            <button @click='updateZLG(item, index)' style='background:#56BE65;color: white;border: 0;border-radius: 5px'>修改</button>
            <button @click='deleteZLG(item.id, index)' style="margin-left: 15px;background:#DC5250;color: white;border: 0;border-radius: 5px">删除</button>
          </td>
        </tr>
        </tbody>
      </table>
    ...
    <div v-show='isshowzlg'>
      <div class='message'>

      </div>
      <div  class='edit'>
        <div class="title">
          <span class='title-name'>重力柜录入框</span>
  ...
  export default{
    ...
    created () {
      this.$nextTick(async () => {
        ...
        const res = await this.$http.post('http://localhost:3030/Cabinet/findCabinetInStorage', formdata, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        })
        this.zlgdata = res.data
        this.get()
      })
    },
    computed: {
...
//        return /^[A-Z][\d]{2,4}$/.test(this.p.location)
    ...
    methods: {
      ...
      get () {
        this.count = this.zlgdata.length
        for (var i = 0; i < Math.ceil(this.zlgdata.length / this.pageSize); i++) {
          this.list[i] = []
          for (var j = i * this.pageSize; j < (i + 1) * this.pageSize && j < this.zlgdata.length; j++) {
            this.list[i].push(this.zlgdata[j])
          }
        }
      },
      showzlgInput1 () {
        this.SHOW_MESSAGE(true)
        this.p = {id: '', number: '', name: '', location: '', status: false}
      },
      updateZLG (item, index) {
        this.SHOW_MESSAGE(true)
      ...
      async searchmethod () {
        if (this.search !== 0) {
          ...
          formdata.append('storageId', this.strogeId)
          const result = await this.$http.post('http://localhost:3030/Cabinet/queryCabinetInStorage', formdata, {
            headers: {
              'Content-Type': 'multipart/form-data'
            }
          })
          this.zlgdata = result.data
          this.get()
        } else {
      ...
      async deleteZLG (id, index) {
        var formdata = new FormData()
        formdata.append('id', id)
        const result = await this.$http.post('http://localhost:3030/Cabinet/deleteCabinet', formdata, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        })
      ...
      async submit () {
        ...
        if (this.numberValue && this.nameValue && this.locationValue) {
          const result = await this.$http.post('http://localhost:3030/Cabinet/saveCabinet', formdata, {
            headers: {
              'Content-Type': 'multipart/form-data'
            }
          })
          if (this.p.id === '') {
            if (result.data.addfail) {
              alert(result.data.addfail)
            } else {
              alert('增加成功')
              ...
              this.zlgdata.unshift(res.data[res.data.length - 1])
              this.get()
            }
          } else if (this.p.id !== '') {
            if (result.data.update === 'ok') {
              ...
              const res = await this.$http.post('http://localhost:3030/Cabinet/findCabinetInStorage', formdata2, {
                headers: {
                  'Content-Type': 'multipart/form-data'
                }
              })
              for (var i = 0; i < res.data.length; i++) {
                for (var key in res.data[i]) {
                  if (res.data[i].id === this.p.id) {
                    this.zlgdata[this.updateIndex] = res.data[i]
                    break
                  }
                }
              }
